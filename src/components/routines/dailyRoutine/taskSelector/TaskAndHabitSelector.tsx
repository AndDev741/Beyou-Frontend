...
const TaskAndHabitSelector = ({ setRoutineSection, index, section, setOpenTaskSelector }: TaskSelectorProps) => {
    ...
    console.log(tasks)
    return (
        <div className="flex flex-col items-center justify-start w-full border-2 border-blueMain rounded-lg p-2 mt-2 bg-white shadow-sm">
            ...
            <div className="w-full flex flex-col items-start justify-evenly">
                <div className="flex flex-col items-center justify-start w-full my-1 mx-2">
                    <h2 className="font-medium">{t("Put the start time of this item")}</h2>
                    <p className="text-sm text-center text-gray-600">{t('Need to fit between the section time')}</p>
                    <input
                        type="time"
                        className="border-2 border-blueMain rounded-lg p-1 mt-1 font-medium"
                ...
                {habits.length === 0 && tasks.length === 0 && (
                    <span className="text-gray-500">{t("No habits or task available, create one")}</span>
                )}
