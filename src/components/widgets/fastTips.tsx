...
export default function FastTips(){
    ...
    return (
        <BaseDiv title={t('Fast Tips')} bigSize={true} >
            {/* Mobile */}
            <p className="md:hidden text-primary font-medium text-center line-clamp-1">{t(fastTipsData.habit.smallPhrase)}</p>
            {/* Desktop */}
            <p className="hidden md:block text-xl text-primary font-medium text-center line-clamp-1">{t(fastTipsData.habit.phrase)}</p>
            <a 
