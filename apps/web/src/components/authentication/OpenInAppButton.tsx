...
function OpenInAppButton({ path, token }: OpenInAppButtonProps) {
    ...
    return (
        <a
            ...
            className="flex items-center justify-center gap-2 px-8 py-3 my-4 bg-accent text-on-accent rounded-card font-semibold text-lg hover:opacity-90 transition-opacity"
        >
            {t("OpenInBeyouApp")}
        </a>
    );
