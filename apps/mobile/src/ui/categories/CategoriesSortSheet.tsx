...
/** value → i18n key, mirroring the web categories SortFilterBar options.
 *  Categories have xp/level but no importance/difficulty. */
...
/** A pill showing the current category sort that opens a bottom-sheet of options.
 *  Sort state lives in the shared `viewFilters` slice (same as web). */
export default function CategoriesSortSheet() {
            ...
            {OPTIONS.map((o) => {
              ...
              return (
                <Pressable
                  key={o.value}
                  onPress={() => {
                    dispatch(setViewSort({ view: 'categories', sortBy: o.value }));
                    setOpen(false);
                  }}
                  accessibilityRole="button"
