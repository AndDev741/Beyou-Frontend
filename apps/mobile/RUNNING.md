...
## Verifying icon / app-name changes on a device

The `android/` directory is gitignored and regenerated by prebuild, and a plain
`npx expo prebuild` keeps whatever stale generated resources it already finds there.
After changing icons, splash images or the app name, regenerate from scratch:
...
The brand rasters themselves come from `tools/make-icons.py` (see its docstring);
`python3 apps/mobile/tools/make-icons.py --check` verifies the committed files.
